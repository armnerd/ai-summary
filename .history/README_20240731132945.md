...
1.ollama其实是一个本地模型的管理软件,里面有很多本地的大模型。
...
export OLLAMA_HOST="1.1.1.1:11434"
#---------------------------------------------------------------------------------------
...
# 海外主机
docker run -d -p 3000:8080 -e OLLAMA_BASE_URL=http://1.1.1.1:11434 -v open-webui:/app/backend/data --name open-webui --restart always ghcr.io/open-webui/open-webui:main

 
# 国内可用的镜像源(修改对应的地址指向)
docker run -d -p 80:8080 -e OLLAMA_BASE_URL=http://1.1.1.1:11434 -v open-webui:/app/backend/data --name open-webui --restart always ghcr.chenby.cn/open-webui/open-webui:main
